...
public sealed partial class CrewMonitoringWindow : FancyWindow
{
    ...
    // DS14-start
    private readonly StationAiTrackEntitySystem _stationAiTrack;
    public event Action<CrewMonitoringConsolePingMode>? PingModeSelected;

    private static readonly CrewMonitoringConsolePingMode[] PingModes =
    [
    ...
    public CrewMonitoringWindow()
    {
        ...
        // DS14-start
        foreach (var mode in PingModes)
        {
            PingModeOption.AddItem(GetPingModeLabel(mode));
        }

        PingModeOption.OnItemSelected += args =>
        {
            PingModeOption.SelectId(args.Id);
            PingModeSelected?.Invoke(PingModes[args.Id]);
        };
        // DS14-end
    }

    // DS14-start
    public void SetPingMode(CrewMonitoringConsolePingMode mode)
    {
        var index = Array.IndexOf(PingModes, mode);
        if (index >= 0)
            PingModeOption.SelectId(index);
    }

    private static string GetPingModeLabel(CrewMonitoringConsolePingMode mode)
    {
        var key = mode switch
        {
    ...
    public void ShowSensors(List<SuitSensorStatus> sensors, EntityUid monitor, EntityCoordinates? monitorCoords, bool serverless = false) // DS14
    {
        ...
        var showAiCameraButtons = CanTrackWithAiCameras(); // DS14
        ...
        foreach (var department in departments)
        {
            ...
            PopulateDepartmentList(departmentSensors, showAiCameraButtons); // DS14
        }
        ...
        if (remainingSensors.Any())
        {
            ...
            PopulateDepartmentList(remainingSensors, showAiCameraButtons); // DS14
        }
    ...
    private void PopulateDepartmentList(IEnumerable<SuitSensorStatus> departmentSensors, bool showAiCameraButtons) // DS14
    {
        ...
        foreach (var sensor in departmentSensors)
        {
            ...
            var sensorButton = new CrewMonitoringButton()
            {
                ...
                Disabled = coordinates == null, // DS14
                HorizontalExpand = true,
            };
            ...
            // DS14-start
            if (showAiCameraButtons)
            {
                var cameraButton = new Button
                {
                    Text = Loc.GetString("crew-monitoring-ui-camera-button"),
                    ToolTip = Loc.GetString("crew-monitoring-ui-camera-button-tooltip"),
                    SetWidth = 58,
                    Margin = new Thickness(8, 0, 0, 0),
                    Disabled = sensor.OwnerUid == NetEntity.Invalid,
                };
    ...
    // DS14-start
    private bool CanTrackWithAiCameras()
    {
        return _player.LocalEntity is { } localEntity &&
               _entManager.HasComponent<StationAiHeldComponent>(localEntity);
    }
    // DS14-end
}
