...
public sealed partial class ShuttleNavControl : BaseShuttleControl
{
    ...
    private void DrawDetectables(DrawingHandleScreen handle, Matrix3x2 gridToView)
    {
        ...
        const float detectableScale = 0.6f;

        const float sqrt2 = 1.41421356f;
        const float detectableRadius = detectableScale * sqrt2;

        Box2 viewBounds = new Box2(-detectableRadius, -detectableRadius, Size.X + detectableRadius, Size.Y + detectableRadius);
        foreach (var detectable in _detectables)
        {
            ...
            {
                case DetectableDrawType.Circle:
                    if (!string.IsNullOrWhiteSpace(detectable.Name))
                    {
                        handle.DrawString(
                            _font,
                ...
                case DetectableDrawType.Square:
                    if (!string.IsNullOrWhiteSpace(detectable.Name))
                    {
                        handle.DrawString(
                            _font,
                ...
                case DetectableDrawType.Entity:
                    if (!string.IsNullOrWhiteSpace(detectable.Name))
                    {
                        handle.DrawString(
                            _font,
