...
public sealed class UnitologyInvitationConditionsSystem : EntitySystem
{
    [Dependency] private readonly NumberObjectiveSystem _number = default!;

    public override void Initialize()
    {
        SubscribeLocalEvent<UnitologyInvitationConditionsComponent, ObjectiveGetProgressEvent>(OnGetProgress);
    }

    private void OnGetProgress(EntityUid uid, UnitologyInvitationConditionsComponent component, ref ObjectiveGetProgressEvent args)
    {
        args.Progress = InvitationProgress(component, _number.GetTarget(uid));
    }

    private float InvitationProgress(UnitologyInvitationConditionsComponent component, int target)
    {
        if (target == 0)
            return 1f;
